document.addEventListener('DOMContentLoaded', () => {
    ...
    window.System = {
        ...
        showNotification: (title, body, icon) => {
            if(Notification.permission === 'granted') {
                ...
                notif.style.cssText = 'position:fixed; top:10px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,0.8); color:white; padding:10px 20px; border-radius:20px; z-index:10000; display:flex; align-items:center; gap:10px; box-shadow:0 4px 10px rgba(0,0,0,0.2); animation: slideDown 0.5s ease;';
    ...
    window.System.showNotification = (title, body, icon, appId) => {
        ...
        if(shouldShowInApp) {
            const notif = document.createElement('div');
            notif.className = 'in-app-notification';
            notif.style.cssText = 'position:fixed; top:10px; left:50%; transform:translateX(-50%); background:rgba(255,255,255,0.95); color:#333; padding:12px 20px; border-radius:25px; z-index:10000; display:flex; align-items:center; gap:12px; box-shadow:0 5px 15px rgba(0,0,0,0.15); animation: slideDown 0.5s ease; cursor:pointer; min-width:280px; backdrop-filter:blur(10px); border:1px solid rgba(0,0,0,0.05);';
