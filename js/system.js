document.addEventListener('DOMContentLoaded', () => {
    ...
    window.System = {
        ...
        resetContext: () => {
            ...
            if(context) {
                if(context.qq) localStorage.setItem('qq_data', context.qq);
                if(context.twitter) localStorage.setItem('twitter_data', context.twitter);
                if(context.instagram) localStorage.setItem('instagram_data', context.instagram);
                if(context.fanfic) localStorage.setItem('fanfic_data', context.fanfic);
                
                if(context.birthday) localStorage.setItem('birthday_date', context.birthday);
                else localStorage.removeItem('birthday_date');
                
                if(context.shop) localStorage.setItem('shop_data', context.shop);
                if(context.forum) localStorage.setItem('forum_data', context.forum);
                if(context.couple) localStorage.setItem('couple_data', context.couple);
                if(context.worldbook) localStorage.setItem('worldbook_data', context.worldbook);
    ...
    // Auto-restore if backup exists (handle refresh during Phone Check Mode)
    const backup = localStorage.getItem('system_backup');
    if (backup) {
        console.log('Found system backup, restoring context...');
        setTimeout(() => {
