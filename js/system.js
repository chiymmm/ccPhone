document.addEventListener('DOMContentLoaded', () => {
    ...
    async function triggerRandomActivity() {
        ...
        const apps = ['qq', 'twitter', 'instagram', 'forum'];
        ...
        try {
            if (app === 'qq' && window.QQApp) {
                ...
                // Twitter doesn't have a specific background trigger, reuse generateActivity but silent?
                // Or create a new one. For now, let's just use generateActivity which shows alerts (simulated notification)
                // Ideally we should have a silent mode.
                // Let's just call generateActivity for now as it simulates user interaction or AI action.
                // But wait, generateActivity is for "Phone Check" or manual generation.
                // We need a "background AI post" method.
                // For simplicity, let's assume generateActivity handles it or we skip.
                // Actually, QQ has triggerRandomActivity.
                // Let's stick to QQ for now as it's the main chat.
                window.QQApp.triggerRandomActivity();
    ...
    window.System = {
        ...
        switchContext: (friend) => {
            ...
            // Mock Friend's Data
            // In a real app, we would load specific data files. 
            // Here we generate a mock context based on the friend's profile.
            ...
            // Mock other apps data if needed, or just let them be empty/default
            // For Fanfic (Diary) and Birthday (App Usage), we might want to clear or set specific data
        ...
        resetContext: () => {
            ...
            if(window.System.originalContext) {
                localStorage.setItem('qq_data', window.System.originalContext.qq);
                localStorage.setItem('twitter_data', window.System.originalContext.twitter);
                localStorage.setItem('instagram_data', window.System.originalContext.instagram);
                localStorage.setItem('fanfic_data', window.System.originalContext.fanfic);
                if(window.System.originalContext.birthday) localStorage.setItem('birthday_date', window.System.originalContext.birthday);
                else localStorage.removeItem('birthday_date');
                
                if(window.System.originalContext.shop) localStorage.setItem('shop_data', window.System.originalContext.shop);
                if(window.System.originalContext.forum) localStorage.setItem('forum_data', window.System.originalContext.forum);
                if(window.System.originalContext.couple) localStorage.setItem('couple_data', window.System.originalContext.couple);
                if(window.System.originalContext.worldbook) localStorage.setItem('worldbook_data', window.System.originalContext.worldbook);
    ...
    setInterval(async () => {
        ...
        if(Math.random() < chance) {
            ...
            // Check if we are in Phone Check Mode
            // Even in Phone Check Mode, we want background activities (e.g. character messaging user)
            // But we need to be careful about context.
            // For now, we allow it, as generateActivity methods usually handle context or use global API keys.
    ...
    window.System.showNotification = (title, body, icon, appId) => {
                ...
                if(appId) {
                    if(appId.startsWith('chat:')) {
                        ...
                        if(window.QQApp) {
                            ...
                            window.QQApp.currentChatType = 'friend'; // Assume friend for now
                            // Find if group
                            const qqData = window.QQApp.store.get();
                            if(qqData.groups.find(g => g.id === chatId)) window.QQApp.currentChatType = 'group';
                            
                            const target = window.QQApp.currentChatType === 'group' ? qqData.groups.find(g=>g.id===chatId) : qqData.friends.find(f=>f.id===chatId);
                            if(target) {
        ...
        if(shouldShowInApp) {
            const notif = document.createElement('div');
            notif.className = 'in-app-notification';
            notif.style.cssText = 'position:absolute; top:10px; left:50%; transform:translateX(-50%); background:rgba(255,255,255,0.95); color:#333; padding:12px 20px; border-radius:25px; z-index:10000; display:flex; align-items:center; gap:12px; box-shadow:0 5px 15px rgba(0,0,0,0.15); animation: slideDown 0.5s ease; cursor:pointer; width:90%; backdrop-filter:blur(10px); border:1px solid rgba(0,0,0,0.05);';
            
            let iconHtml = '';
            if(icon && icon.startsWith('data:')) iconHtml = `<div style="width:36px;height:36px;background:url('${icon}') center/cover;border-radius:50%;"></div>`;
            else if(icon) iconHtml = `<div style="width:36px;height:36px;background:url('${icon}') center/cover;border-radius:50%;"></div>`;
            else iconHtml = `<div style="width:36px;height:36px;background:#eee;border-radius:50%;display:flex;align-items:center;justify-content:center;"><i class="fas fa-bell" style="color:#666;"></i></div>`;

            notif.innerHTML = `
                ${iconHtml}
                <div style="flex:1;">
                    <div style="font-weight:bold;font-size:14px;margin-bottom:2px;">${title}</div>
                    <div style="font-size:12px;color:#666;line-height:1.2;">${body}</div>
                </div>
