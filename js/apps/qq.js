    ...
    if(!panel) {
        ...
        document.getElementById('emojiInput').onchange = async (e) => {
            if(e.target.files[0]) {
                window.Utils.showCustomDialog({
                    title: '添加表情包',
                    content: '<div style="text-align:center;margin-bottom:15px;"><img id="emojiPreviewImg" style="max-width:120px;max-height:120px;border-radius:12px;border:2px solid #eee;"></div>',
                    inputs: [{ id: 'meaning', placeholder: '这个表情是什么意思？(例如: 开心/嘲讽/无语)' }],
                    buttons: [
                        { text: '取消', class: 'cancel', value: false },
                    ...
                    if(res.action && res.inputs.meaning) {
                        try {
                            const base64 = await window.Utils.compressImage(await window.Utils.fileToBase64(e.target.files[0]), 200, 0.8);
                            const id = await window.db.saveImage(base64);
                            this.store.update(d => {
                                if(!d.emojis) d.emojis = [];
                                d.emojis.push({id: window.Utils.generateId('emo'), url: id, meaning: res.inputs.meaning});
                            });
                ...
                reader.onload = (ev) => {
                    setTimeout(() => {
                        const img = document.getElementById('emojiPreviewImg');
