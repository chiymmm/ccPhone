...
`;
    ...
    async summarizeMemory(chatId, force = false) {
        ...
        // 强化第三人称旁观者提示词
        const prompt = `【记忆总结系统 - 第三人称旁观者模式】

⚠️ 重要指令：你现在是一个冷静客观的第三方观察者/记录员禁止扮演任何角色禁止使用第一人称。

你需要以旁观者身份严肃、客观地总结以下对话中的关键信息。
...
总结要求：
1. 使用第三人称描述（例如："用户提到..."、"${target.name}表示..."）
2. 提取关键事实：重要事件、约定、承诺、用户喜好、情感变化
3. 按时间线或重要性排序
4. 简洁明了每条记忆一行，用"•"开头
5. 禁止角色扮演禁止使用"我"、"你"等第一/第二人称
6. 禁止添加主观情感评价只记录客观事实

示例格式：
• 用户向${target.name}提到自己喜欢吃草莓
• ${target.name}答应周末陪用户去看电影
• 双方确立了情侣关系（日期：xxxx）
...
请开始总结：`;
    ...
    async openMurmur() {
        ...
        if(!modal) {
            ...
            modal.innerHTML = '<div class="sub-header"><button class="back-btn" onclick="document.getElementById(\'murmurModal\').style.display=\'none\'"><i class="fas fa-chevron-left"></i></button><span class="sub-title">碎碎念</span><button class="menu-btn" id="refreshMurmur"><i class="fas fa-sync-alt"></i></button></div><div class="sub-content" id="murmurList" style="padding:15px;"></div>';
    ...
    async openMemo() {
        ...
        if(!modal) {
            ...
            modal.id = 'memoModal';
            modal.className = 'sub-page';
            modal.innerHTML = '<div class="sub-header"><button class="back-btn" onclick="document.getElementById(\'memoModal\').style.display=\'none\'"><i class="fas fa-chevron-left"></i></button><span class="sub-title">备忘录</span><button class="menu-btn" id="addMemoBtn"><i class="fas fa-plus"></i></button></div><div class="sub-content" id="memoList" style="padding:15px;"></div>';
            document.body.appendChild(modal);
        }
        modal.style.display = 'flex';
    ...
    renderMemos(friend) {
        ...
        if(memos.length === 0) {
            list.innerHTML = '<div style="text-align:center;padding:40px;color:#ccc;"><i class="fas fa-sticky-note" style="font-size:36px;margin-bottom:15px;"></i><br>还没有备忘~</div>';
            return;
        }
        list.innerHTML = '';
        const self = this;
        memos.forEach(function(m, i) {
            ...
            div.innerHTML = '<div class="memo-sticky ' + (m.type === 'anniversary' ? 'pink' : '') + '"><div class="memo-title">' + m.title + '</div><div class="memo-content">' + m.content + '</div>' + dateHtml + '<div class="memo-actions"><i class="fas fa-pencil-alt" data-index="' + i + '"></i><i class="fas fa-trash-alt" data-index="' + i + '"></i></div></div>';
            div.querySelector('.fa-pencil-alt').onclick = function() { self.editMemo(parseInt(this.dataset.index)); };
            div.querySelector('.fa-trash-alt').onclick = function() { self.deleteMemo(parseInt(this.dataset.index)); };
            list.appendChild(div);
        });
    }
    ...
    editMemory(index) {
        ...
        const memories = (friend.memory?.summary || '').split('\n').filter(s => s.trim());
        const current = memories[index] || '';
        window.Utils.showCustomDialog({ title: '编辑记忆', inputs: [{ id: 'content', type: 'textarea', value: current.replace(/^[•\-]\s*/, '') }], buttons: [{ text: '保存', class: 'confirm', value: 'save' }, { text: '删除', class: 'cancel', value: 'delete' }] }).then(res => {
            if(res.action === 'save') { memories[index] = '• ' + res.inputs.content; this.store.update(d => { const f = d.friends.find(x => x.id === this.currentChatId); if(f) f.memory.summary = memories.join('\n'); }); this.openMemoryEditor(); }
            else if(res.action === 'delete') { memories.splice(index, 1); this.store.update(d => { const f = d.friends.find(x => x.id === this.currentChatId); if(f) f.memory.summary = memories.join('\n'); }); this.openMemoryEditor(); }
        });
    }
