...
setTimeout(() => {
    ...
    const currentIsGroup = true; // 这个分支就是群聊
...
if(!isGroup) {
    ...
    if(currentFriend && (!currentFriend.wallet || !currentFriend.wallet.initialized)) {
        needWalletInit = true;
    }
}
...
let groupCreationPrompt = '';
if(pendingGroup && pendingGroup.creatorId === this.currentChatId) {
    groupCreationPrompt = `  "newGroupMembers": [
    {"name": "成员1姓名", "relation": "与${pendingGroup.creatorName}的关系", "persona": "人设50字"},
    {"name": "成员2姓名", "relation": "关系", "persona": "人设50字"}
  ],
  // 🔴 必填！为群聊"${pendingGroup.groupName}"生成${pendingGroup.memberCount - 1}个新成员
`;
}

// 检查是否有待互动的朋友圈
const pendingMomentInteractions = data.pendingMomentInteractions || [];
let momentInteractionPrompt = '';
if(pendingMomentInteractions.length > 0) {
    const latestPending = pendingMomentInteractions[0];
    const targetMoment = data.moments.find(m => m.id === latestPending.momentId);
    if(targetMoment) {
        const interactingFriends = data.friends.filter(f => f.id !== targetMoment.userId).slice(0, 3);
        const friendsJson = interactingFriends.map(f => `{"name": "${f.name}", "like": true, "comment": "评论内容"}`).join(',\n      ');
        momentInteractionPrompt = `  "momentInteractions": {
...
let otherFriendsInfo = '';
if(shouldGenerateOtherChats) {
    otherFriendsInfo = otherFriendsForChat.map(f => `${f.name}(${f.persona?.substring(0, 50) || '普通朋友'})`).join('\n');
}
...
${autoGroupStatus ? `  "groupStatus": {
    "atmosphere": "群聊当前氛围",
    "topic": "正在讨论的话题",
    "activity": "成员们在做什么"
  },` : ''}
${autoMemberStatus ? `  "memberStatuses": [
    {"name": "发言成员名", "thought": "在想什么", "status": "状态词"}
  ],` : ''}
${needWalletInit ? `  "walletInit": {
    "balance": 1000,
    "reason": "经济状况说明"
  },` : ''}
${groupCreationPrompt}
${momentInteractionPrompt}
${shouldGenerateOtherChats ? `  "otherChats": [
    {
      "targetName": "聊天对象姓名（可以是任何人：朋友/同学/家人/同事/网友等）",
      "targetRelation": "与${target.name}的关系（如：闺蜜/死党/同学/网友/家人等）",
      "targetPersona": "对方简短人设（20字以内如果是新角色必填）",
      "messages": [
        {"speaker": "${target.name}", "content": "发起话题"},
        {"speaker": "对方名字", "content": "回复内容"}
      ]
    }
  ],
  // 💡 聊天对象可以自由创造不限于用户好友列表。创造符合${target.name}社交圈的真实人物。
` : ''}

${shouldGenerateMoment ? `  "moment": "根据心情生成朋友圈内容",` : ''}
  "murmur": ""
}
</EXTRA_DATA>
