...
if(!isGroup && enableOtherChats) {
    otherFriendsForChat = data.friends.filter(f => f.id !== this.currentChatId).slice(0, 3);
}
...
{
${autoStatus ? `  "status": {
    "thought": "（必填）你现在具体在想什么一句话",
    "status": "（必填）状态词如:发呆/开心/无聊/忙碌/想TA",
    "action": "（必填）你正在做什么",
    "todo": "（必填）接下来想做什么"
  },` : ''}
${autoGroupStatus ? `  "groupStatus": {
    "atmosphere": "（必填）群聊当前氛围",
    "topic": "（必填）正在讨论的话题",
    "activity": "（必填）成员们在做什么"
  },` : ''}
...
${needWalletInit ? `  "walletInit": {
    "balance": （必填）根据你的人设生成初始余额数字如学生500-3000/白领3000-10000/富人10000+,
    "reason": "（必填）简述经济状况"
  },` : ''}
${(otherFriendsForChat.length > 0 && Math.random() < 0.3) ? `  "otherChats": [
    {
      "targetName": "${otherFriendsForChat[Math.floor(Math.random() * otherFriendsForChat.length)].name}",
      "messages": [
        {"speaker": "你的名字", "content": "消息1"},
        {"speaker": "对方名字", "content": "回复1"},
        {"speaker": "你的名字", "content": "消息2"}
      ]
    }
  ],` : ''}
  "murmur": "（可选）你的内心独白碎碎念留空则不生成",
  "moment": "（可选）你想发的朋友圈内容留空则不生成"
}
</EXTRA_DATA>

⚠️ 严格要求：
1. 【必须】在对话回复结束后立即添加 <EXTRA_DATA> 块
2. 【必须】生成所有标记为"必填"的字段不能为空不能省略
3. 【禁止】省略任何必填字段
4. 【禁止】只输出对话不输出 <EXTRA_DATA>
5. 状态内容必须根据当前对话情境生成要具体不要敷衍
6. JSON格式必须正确不要有语法错误
7. 数字类型不要加引号

如果你的回复中没有包含完整的 <EXTRA_DATA> 块将被视为任务失败！
`;
            ...
            this.store.update(d => {
                ...
                if(f) {
                    ...
                    extraData.otherChats.forEach(chatSession => {
                        ...
                        const targetFriend = d.friends.find(x => x.name === chatSession.targetName);
                        ...
                        chatSession.messages.forEach(msg => {
                            ...
                            // 🔴 动态更新钱包：检查对话中是否有转账
                            const transferMatch = msg.content.match(/转账|红包|(\d+)元/);
                            if(transferMatch) {
                                ...
                                if(amountMatch) {
                                    ...
                                    if(!isNaN(amt) && amt > 0) {
                                        // 判断是收入还是支出
                                        const isReceiving = msg.speaker !== f.name;
                                        if(f.wallet && f.wallet.enabled) {
                                            if(isReceiving) {
                                                f.wallet.balance = (parseFloat(f.wallet.balance) + amt).toFixed(2);
                                                f.wallet.history.unshift({
                                                    date: new Date().toLocaleString(),
                                            ...
                                            console.log(`✅ ${f.name} 钱包变动：${isReceiving ? '+' : '-'}¥${amt}`);
                                        }
                                    }
                                }
                            }
                        ...
                        console.log(`✅ ${f.name} 和 ${chatSession.targetName} 的对话已保存`);
                    });
...
// 🔴 新增：如果没有EXTRA_DATA块但开启了状态栏则生成默认状态
