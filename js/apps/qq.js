...
handleCreateGroup() {
    ...
    // ✅ 新增：如果不是偷看模式自动把用户加入成员列表
    const isSpectator = document.getElementById('isSpectator').checked;
    if(!isSpectator && !members.includes('user')) {
        members.unshift('user'); // 用户作为第一个成员
    }
    ...
    const group = {
        ...
        members: members, // 成员ID列表
        owner: 'user', // ✅ 新增：群主（默认是用户）
        admins: [], // ✅ 新增：管理员列表
        memberRoles: {}, // ✅ 新增：成员头衔 {memberId: "头衔"}
        background: '', // ✅ 新增：群聊背景
...
if(isGroup) {
    ...
    const memberListHtml = members.map(m => {
        ...
        return `
            <div 
class="member-item" style="display:flex;align-items:center;padding:10px;border-bottom:1px solid #f5f5f5;"
>
                <div style="width:40px;height:40px;border-radius:50%;background:#eee;margin-right:10px;"></div>
                <div style="flex:1;">
...
` : ''}
                ...
                ${m.
id !== 'user' ? `<button class="action-btn secondary" onclick="window.QQApp.manageMember('${m.id}')" style="padding:5px 10px;font-size:12px;">管理</button>` : ''
}
            </div>
        `;
    }).join('');
    ...
    html = `
        <div 
class="form-group"><label>群名称</label><input id="editName" value="${target.name}"></div>
        ...
        <div style="margin:15px 0;">
            ...
            <div style="max-height:300px;overflow-y:auto;border:1px solid #f0f0f0;border-radius:8px;">
                ${memberListHtml}
            </div>
        </div>
    ...
    // ========== 事件绑定 ==========
    setTimeout(() => {
        document.getElementById('saveGroupSettings').onclick = () => {
            this.store.update(d => {
        ...
        document.getElementById('btnAddMember').onclick = () => this.addGroupMember();
    }, 50);

    return; // ✅ 重要：群聊直接返回不执行后面的好友设置代码
}
...
- 记住：你认为用户看不到这条消息`;
    ...
    try {
        ...
        this.store.update(d => {
            const f = d.friends.find(x => x.id === friend.id);
            if(f) {
                if(!f.blockedMessages) f.blockedMessages = []; // 确保数组存在
    ...
    if(messages.length === 0) {
        list.innerHTML = `
            <div style="text-align:center;padding:60px 20px;color:#ccc;">
                ...
                还没有消息<br>
                <button class="action-btn" id="genFirstMsg" style="margin-top:20px;">
                    让TA发条消息
                </button>
            </div>
        `;
        ...
        // ✅ 修复：延迟绑定确保DOM已加载
        setTimeout(() => {
            const btn = document.getElementById('genFirstMsg');
