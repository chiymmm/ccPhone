...
async uploadFile(type) {
    if(type === 'novel') {
        ...
        window.Utils.showCustomDialog({
            ...
            `,
            inputs: [
                { id: 'novelTitle', placeholder: '小说标题（可选）' }
            ],
            buttons: [
                { text: '取消', class: 'cancel', value: false },
                { text: '发送', class: 'confirm', value: true }
        ...
        window.Utils.showCustomDialog({
            ...
            `,
            inputs: [
                { id: 'musicTitle', placeholder: '歌曲名称（可选）' }
            ],
            buttons: [
                { text: '取消', class: 'cancel', value: false },
                { text: '发送', class: 'confirm', value: true }
            ...
            if(res.action) {
                ...
                if(fileArea && fileArea.style.display !== 'none') {
                    ...
                    if(fileInput && fileInput.files[0]) {
                        ...
                        musicId = await window.db.saveImage(base64); // 复用图片存储
                    }
                } else if(urlArea && urlArea.style.display !== 'none') {
                    // 🔴 修复：URL模式
...
const group = {
    ...
    memberStatuses: {}, // ✅ 新增：成员状态 {memberId: {thought, status, action, todo}}
    settings: {
        contextLimit: 15,
        memorySync: true,
        timeSense: false,
        ...
        if(res.action && res.inputs.meaning) {
            ...
            if(urlInput && urlInput.value.trim()) {
                ...
                try {
                    ...
                    imageData = await window.Utils.compressImage(base64, 400, 0.95);
                } catch(e) {
                    // 如果跨域失败直接保存URL
                    imageData = url;
                }
    ...
    window.Utils.showCustomDialog({
        ...
        content: `
            <div style="margin-bottom:15px;">
                <label style="display:block;margin-bottom:5px;">头衔</label>
                <input id="memberRole" value="${role}" placeholder="例如：管理员、元老、活跃分子..." style="width:100%;padding:8px;border:1px solid #eee;border-radius:5px;">
            </div>
        `,
        buttons: [
            { text: isAdmin ? '取消管理员' : '设为管理员', class: 'secondary', value: 'admin' },
            { text: '转让群主', class: 'secondary', value: 'transfer' },
        ...
        if(res.action === 'save') {
            ...
            this.store.update(d => {
                ...
                if(g) {
                    ...
                    if(isAdmin) g.admins = g.admins.filter(x => x !== memberId);
                    else g.admins.push(memberId);
                }
            });
            ...
            window.Utils.showCustomDialog({
                title: '转让群主',
                content: `确定将群主转让给 ${member.name} 吗？`,
                buttons: [
                    { text: '取消', class: 'cancel', value: false },
                    { text: '确定', class: 'confirm', value: true }
                ]
...
setTimeout(() =>
 {
    this.triggerFriendsInteraction(friend.id
, momentId);
}, 
