...
async sendVoiceMessage(content, duration, isReal) {
    ...
    // AI回复时使用转文字内容
    if(transcription && transcription !== '[语音转文字失败]' && transcription !== '[未配置STT API]') {
        this.handleAIResponse(null, transcription);
    } else if(isReal) {
        this.handleAIResponse(null, '[用户发送了语音消息]');
    } else {
...
();
    ...
    if(!target) return window.Utils.showToast('请先选择好友'); // 🔴 添加这行检查
...
const sendVc = async (
    ...
    if
(!input) {
        console.warn('VC input not found'
);
        return; // ✅ 修复：输入框不存在时返回
    }
...
请以记录员身份输出关键记忆点（5-10条）：`;
        ...
        // 🟢 清理通话上下文（可选保留供下次使用）
        // localStorage.removeItem(`vc_context_${this.currentChatId}`);
        ...
        window.Utils.showCustomDialog({
            ...
            content: `
                ...
                <div id="novelTextArea" class="upload-area" style="display:none;">
                    <textarea id="novelTextInput" placeholder="粘贴小说内容..." style="width:100%;height:150px;padding:10px;border:1px solid #eee;border-radius:8px;resize:vertical;"></textarea>
                </div>
            `,
            inputs: [{ id: 'novelTitle', placeholder: '小说标题（可选）' }],
            buttons: [
                { text: '取消', class: 'cancel', value: false },
                { text: '发送', class: 'confirm', value: true }
            ]
        }).then(async (res) => {
            if(res.action && res.inputs.novelTitle) {
                let content = '';
                let title = res.inputs.novelTitle || '未命名小说';

                const fileArea = document.getElementById('novelFileArea');
                const urlArea = document.getElementById('novelUrlArea');
...
,
        subType: 'novel'
,
        data: { title: title, content: content }, // 🔴 改为对象结构
        timestamp: Date.now
(),
        status: 'normal'
    });
});
                window.Utils.showToast('已发送');
            }
        });
        ...
        setTimeout(() => {
            document.querySelectorAll('.utt-btn').forEach(btn => {
                btn.onclick = () => {
                    ...
                    document.getElementById('novelFileArea').style.display = type === 'file' ? 'block' : 'none';
                    document.getElementById('novelUrlArea').style.display = type === 'url' ? 'block' : 'none';
                    document.getElementById('novelTextArea').style.display = type === 'text' ? 'block' : 'none';
                };
            });
            ...
            dropzone.ondrop = (e) => {
                ...
                if(e.dataTransfer.files[0]) {
                    ...
                    dropzone.innerHTML = `<i class="fas fa-check-circle"></i><span>已选择: ${e.dataTransfer.files[0].name}</span>`;
                }
            };

            fileInput.onchange = (e) => {
                if(e.target.files[0]) {
                    dropzone.innerHTML = `<i class="fas fa-check-circle"></i><span>已选择: ${e.target.files[0].name}</span>`;
                }
            };
        }, 100);
        ...
        window.Utils.showCustomDialog({
            ...
            `,
            inputs: [{ id: 'musicTitle', placeholder: '歌曲名称（可选）' }],
            buttons: [
                { text: '取消', class: 'cancel', value: false },
                { text: '发送', class: 'confirm', value: true }
            ]
        }).then(async (res) => {
            if(res.action && res.inputs.musicTitle) {
                let musicId = '';
                let title = res.inputs.musicTitle || '未命名歌曲';

                const fileArea = document.getElementById('musicFileArea');
                const urlArea = document.getElementById('musicUrlArea');

                if(fileArea && fileArea.style.display !== 'none') {
                    // 文件模式
                    const fileInput = document.getElementById('musicFileInput');
                    if(fileInput && fileInput.files[0]) {
                        const base64 = await window.Utils.fileToBase64(fileInput.files[0]);
                    ...
                    if(urlInput && urlInput.value.trim()) {
                        musicId = urlInput.value.trim();
                    }
                }

                if(!musicId) {
                    window.Utils.showToast('请选择音乐');
                    return;
                }
...
,
        subType: 'music'
,
        data: { title: title, fileId: musicId }, // 🔴 改为对象结构
        timestamp: Date.now
(),
        status: 'normal'
    });
        ...
        setTimeout(() => {
            document.querySelectorAll('.utt-btn').forEach(btn => {
                btn.onclick = () => {
                    ...
                    const type = btn.dataset.type;
                    document.getElementById('musicFileArea').style.display = type === 'file' ? 'block' : 'none';
                    document.getElementById('musicUrlArea').style.display = type === 'url' ? 'block' : 'none';
                };
            });
...
();
    ...
    const renderChapter = () => {
        ...
        this.updateUserActivity('novel', {
            ...
            currentContent: chapters[currentChapter].substring(0, 500) // 当前章节前500字
        });
    };
    ...
    float.querySelector('#closeNovel').onclick = () => {
        float.remove();
        this.updateUserActivity('novel', null); // 清除状态
    };
    ...
    float.querySelector('#minNovel').onclick = () => {
        ...
        float.querySelector('.novel-controls').style.display = float.classList.contains('minimized') ? 'none' : 'flex';
        float.querySelector('.novel-progress-bar').style.display = float.classList.contains('minimized') ? 'none' : 'block';
    };
...
// 📍 位置：openChatSettings() 方法中的 if(isGroup) 代码块
// 🔴 将整个 if(isGroup) { ... return; } 替换为以下代码：
...
;
    const memberStatus = (target.memberStatuses || {})[m.id]; // ✅ 获取成员状态

    let badge = ''
;
    if(isOwner) badge = '<span style="background:#ff9f43;color:#fff;padding:2px 6px;border-radius:4px;font-size:10px;margin-left:5px;">群主</span>'
;
    else if(isAdmin) badge = '<span style="background:#5f9ea0;color:#fff;padding:2px 6px;border-radius:4px;font-size:10px;margin-left:5px;">管理</span>'
;
...
;
    if(memberStatus && memberStatus.status
        ...
        statusHint = 
`<div style="font-size:10px;color:#999;margin-top:2px;">📍 ${memberStatus.status}</div>`
;
    }
...
`
        <div class="member-item" style="display:flex;align-items:center;padding:10px;border-bottom:1px solid #f5f5f5;">
            <div style="width:40px;height:40px;border-radius:50%;background:#eee;margin-right:10px;cursor:pointer;" onclick="window.QQApp.viewMemberStatus('
${m.id}
')"></div>
            <div style="flex:1;cursor:pointer;" onclick="window.QQApp.viewMemberStatus('
${m.id}
')">
...
${m.name}${badge}
            ...
            ${m.id !== 'user' ? `<button class="action-btn secondary" onclick="window.QQApp.manageMember('${m.id}')" style="padding:5px 10px;font-size:12px;">管理</button>` : ''}
        </div>
    `
;
}).
    ...
    content.innerHTML = `
        ...
        <div style="margin:15px 0;">
            <div style="font-weight:bold;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;">
                ...
                <button class="action-btn secondary" id="btnAddMember" style="padding:5px 10px;font-size:12px;"><i class="fas fa-user-plus"></i> 添加</button>
            </div>
...
forEach(match =>
                    ...
                    const member = group.members.map(mid =>
 {
                        ...
                        return data.friends.find(f => f.id
 === mid);
                    }).
find(m => m && m.name
 === name);

                    if(member) atMembers.push(member.name
);
                });
...
replace(/@(\S+)/g, (match, name) =>
 {
                        return atMembers.includes(name) ? `<span style="color:#576b95;">@${name}</span>`
 : match;
                    });
                }
            }
...
if(type === 'group' && text.includes('@')) {
        ...
        if(atMatches) {
            atMatches.forEach(match => {
                ...
                const member = group.members.map(mid => {
                    ...
                    return data.friends.find(f => f.id === mid && f.name === name);
                }).find(Boolean);
    ...
    if(needsTranslation) {
        ...
        if(apiConfig.chatApiKey) {
            try {
                ...
                translation = translation.trim(); // 🔴 修复：去除多余空格
            } catch(e) {
    ...
    const msg = {
        ...
        content: text,
        type: 'text',
        translation: translation, // 🔴 修复：正确保存翻译
        timestamp: Date.now(),
        status: 'normal'
    };
...
请使用详细的小说描写风格回复要求：
- 字数控制在500-3000字之间
- 必须包含：环境描写、动作描写、神态描写、心理活动
- 使用第三人称视角
- 描写要细腻生动
- 对话要自然融入叙述中

【示例】
房间里只剩下空调运转的低鸣声。她坐在床边手机屏幕的光映在她微微泛红的脸颊上。刚才的对话还在脑海里回荡让她的心跳有些不受控制。

"真是的..."她小声嘀咕着把手机扣在了床上然后整个人向后倒去陷进柔软的被子里。天花板上的吊灯散发着温暖的黄光她盯着看了一会儿嘴角不自觉地上扬。\n`
;
...
`
        <button class="moment-delete-btn" onclick="window.QQApp.deleteMoment(
${m.id}
)" style="
            position:absolute;
            top:10px;
...
innerHTML = 
`
        <div class="moment-avatar" style="background-image:url('
${avatar}
')"></div>
...
${new Date(m.timestamp).toLocaleString()}
                ...
                <div class="moment-actions">
                    <i class="far fa-heart" onclick="window.QQApp.likeMoment(
${m.id}
)"></i>
                    <i class="far fa-comment" onclick="window.QQApp.commentMoment(
${m.id}
)"></i>
                </div>
            </div>
...
600
        ...
        div.
onmouseup = div.ontouchend = div.onmouseleave = () => clearTimeout
(pressTimer);
    }
...
startBackgroundTasks() {
    ...
    setInterval(() => {
        ...
        data.friends.forEach(friend => {
            const freq = friend.settings?.momentFrequency || 'auto';

            // 跳过自动模式和从不发布
            if(freq === 'auto' || freq === 'never') return;

            // 检查上次发布时间
            if(!friend.lastMomentTime) friend.lastMomentTime = 0;
            const timeSinceLastMoment = now - friend.lastMomentTime;

            let shouldPost = false;

            switch(freq) {
                case 'high':
                    shouldPost = timeSinceLastMoment > 300000; // 5分钟
                    break;
                case 'medium':
                    shouldPost = timeSinceLastMoment > 900000; // 15分钟
                    break;
                case 'low':
                    shouldPost = timeSinceLastMoment > 1800000; // 30分钟
                    break;
            }
        ...
        if(res.action && res.inputs.meaning) {
            ...
            if(urlInput && urlInput.value.trim()) {
                ...
                try {
                    ...
                    imageData = await window.Utils.compressImage(base64,400, 0.98); // 从0.95改为0.98
                } catch(e) {
                    // 如果跨域失败直接保存URL
                    imageData = url;
                }
...
renderMemos(friend) {
    ...
    const novelData = msg.data;
    if(novelData && novelData.title && novelData.content) {
        this.openNovelReader(novelData.title, novelData.content);
    }
}
else if(subType === 'music') {
    const musicData = msg.data;
    if(musicData && musicData.title && musicData.fileId) {
        this.openMusicPlayer(musicData.title, musicData.fileId);
    }
}
