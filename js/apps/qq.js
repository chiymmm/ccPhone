...
();
            if(e.name === 'AbortError'
                ...
                window.Utils.showToast('请求超时请检查网络'
);
            } 
else
 {
                window.Utils.showToast(`链接错误: ${e.message}`
);
            }
            return
;
...
// 在 openNovelReader() 的 renderChapter() 函数中添加：
const renderChapter = (
) => {
    const contentEl = document.getElementById('novelContent'
);
    contentEl.
innerHTML = `<div class="novel-chapter">${chapters[currentChapter].replace(/\n/g, '<br>')}</div>`
;
    contentEl.
scrollTop = 0
;
...
,
                ...
                scrollProgress: scrollPercent, // 🔴 当前章节滚动进度
                currentContent: chapters[currentChapter].substring(0, 500
),
