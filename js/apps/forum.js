...
class ForumApp {
    ...
    initUI() {
        ...
        header.innerHTML = `
            ...
            <div style="display:flex;gap:10px;margin-left:10px;align-items:center;">
                <div id="forumPostBtn" style="cursor:pointer;display:flex;" title="发帖"><i class="fas fa-pen"></i></div>
        ...
        document.getElementById('forumPostBtn').onclick = () => this.showPostEditor();

        // Add Floating Action Button (FAB) for generation
        if (!document.getElementById('forumGenBtn')) {
            const fab = document.createElement('div');
            fab.className = 'forum-fab';
            fab.id = 'forumGenBtn';
            fab.innerHTML = '<i class="fas fa-plus"></i>';
            fab.onclick = () => this.handleGenerate();
            document.getElementById('forumApp').appendChild(fab);
        }
        ...
        if(posts.length === 0) {
            list.innerHTML = `
                <div style="text-align:center;padding:50px;color:#999;display:flex;flex-direction:column;align-items:center;gap:20px;">
                    <p>暂无帖子</p>
                    <button class="action-btn" onclick="window.ForumApp.handleGenerate()" style="width:auto;padding:10px 30px;">
                        <i class="fas fa-magic"></i> 生成帖子
                    </button>
                </div>
            `;
            return;
    ...
    async generatePosts(boardId, query = null) {
        ...
        let prompt = `基于世界观"${settings.worldSetting}"，在"${boardName}"板块生成 8-10 个论坛帖子。
        ${memoryPrompt}`;
        if(query) prompt += `\n帖子内容必须与关键词 "${query}" 相关。`;
    ...
    async generateBoards() {
        ...
        finally { btn.className = 'forum-fab'; btn.innerHTML = '<i class="fas fa-plus"></i>'; }
    }
                    ...
                    },
                    {
                        ...
                        handler: () => {
                            ...
                            tData.tweets.unshift({
                                id: Date.now(), accountId: 'main', text: `转发帖子: ${post.title}\n${post.content.substring(0, 50)}...`, 
                                time: Date.now(), likes: 0, retweets: 0, replies: 0, images: [], comments: []
                            });
    ...
    showPostEditor() {
        ...
        modal.innerHTML = `
            <div class="sub-header">
                <button class="back-btn" id="closePostEditor">取消</button>
                <span class="sub-title">发布帖子</span>
                <button class="send-btn" id="doPostBtn">发布</button>
            </div>
            <div class="sub-content form-content">
                <div class="form-group">
                    <label>标题</label>
                    <input type="text" id="postTitleInput" placeholder="请输入标题">
                </div>
                <div class="form-group">
                    <label>内容</label>
                    <textarea id="postContentInput" placeholder="分享你的想法..." style="height:200px;"></textarea>
                </div>
                <div class="form-group">
                    <label>配图 (可选)</label>
                    <div style="display:flex;gap:10px;align-items:center;">
                        <button class="action-btn secondary" id="uploadPostImgBtn" style="width:auto;">上传图片</button>
                        <span id="postImgName" style="color:#999;font-size:12px;"></span>
                    </div>
                    <input type="file" id="postImgInput" hidden accept="image/*">
                    <div id="postImgPreview" style="margin-top:10px;max-width:100%;max-height:200px;overflow:hidden;border-radius:8px;display:none;">
        ...
        modal.querySelector('#closePostEditor').onclick = () => modal.remove();
        
        modal.querySelector('#uploadPostImgBtn').onclick = () => document.getElementById('postImgInput').click();
        
        document.getElementById('postImgInput').onchange = async (e) => {
            const file = e.target.files[0];
            if (file) {
                document.getElementById('postImgName').innerText = file.name;
                const base64 = await window.Utils.fileToBase64(file);
                imgData = await window.Utils.compressImage(base64);
        ...
        modal.querySelector('#doPostBtn').onclick = async () => {
            ...
            let imgId = null;
            if(imgData) {
                imgId = await window.db.saveImage(imgData);
            }

            const data = this.store.get();
            const newPost = {
                id: window.Utils.generateId('post'),
                boardId: this.currentBoardId || 'general',
                title: title,
                content: content,
    ...
    openMarketItem(item) {
        ...
        modal.innerHTML = `
            ...
            <div style="padding:10px;background:#fff;border-top:1px solid #eee;display:flex;gap:10px;align-items:center;">
                ...
                <button class="shop-btn buy" id="marketBuyBtn" style="flex:2;">立即购买</button>
            </div>
        `;
        document.getElementById('forumApp').appendChild(modal);
