...
class CoupleApp {
    ...
    renderDiaries() {
        ...
        diaries.forEach(d => {
            ...
            div.innerHTML = `
                <div class="diary-date">${new Date(d.time).toLocaleString()} <span style="float:right;font-weight:bold;">${isMe ? '我' : partner.name}</span></div>
        ...
        try {
            if(action === 0) {
                ...
                if(content) {
                    ...
                    if(Notification.permission === 'granted') new Notification(partner.name, {body: '写了一篇新日记'});
                }
            }
        } catch(e) {
