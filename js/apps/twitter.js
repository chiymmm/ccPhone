...
async renderHome() {
    ...
    list.innerHTML = '<div style="padding:30px;text-align:center;"><i class="fas fa-spinner fa-spin"></i></div>';
...
if(t.isRetweet && t.originalTweet) {
    ...
    if (!account && t.isAI) {
        ...
        if(boundRole || enabledRole) {
            avatarSource = 'qq';
            account.qqId = boundRole ? boundRole.qqId : enabledRole.qqId;
        }
    }
    ...
    // 获取正确的头像
    if(account.avatar && account.avatar.startsWith('img_')) {
        avatar = await window.db.getImage(account.avatar);
    } else if(account.avatar && account.avatar.startsWith('data:')) {
        avatar = account.avatar;
    } else if(account.avatar && account.avatar.startsWith('http')) {
        avatar = account.avatar;
    } else {
        // 根据来源获取头像
        avatar = await window.Utils.getCharacterAvatar(account, avatarSource);
    }
...
// 在 createTweetElement 方法中，找到 let quoteHtml = ''; 这一行
// 在它前面插入以下投票渲染代码：
...
// 找到 div.innerHTML 赋值的地方，确保包含 pollHtml：
...
div.querySelector('.like-btn').onclick = (e) => {
    e.stopPropagation();
    const tweetId = t.id;  // ✅ 在这里定义 tweetId
...
saveSettings() {
    // 获取所有输入元素
    const worldSettingEl = document.getElementById('tWorldSetting');
    const postMemoryEl = document.getElementById('tPostMemory');
    const memoryIsolationEl = document.getElementById('tMemoryIsolation');
    ...
    const postMemory = parseInt(postMemoryEl.value) || 0;
    const memoryIsolation = memoryIsolationEl.checked;
    ...
    // 关闭设置页面
    const settingsPage = document.getElementById('tSettingsPage');
...
async renderProfile(target) {
    ...
    if(avatar && avatar.startsWith('img_')) {
        ...
        avatar = await window.Utils.getCharacterAvatar(profileData, avatarSource);
    }
        ...
        if(Array.isArray(tweets) && tweets.length > 0) {
            ...
            this.store.update(d => {
                ...
                d.cachedProfiles[profileData.handle] = {
                    ...
                    avatar: profileData.avatar,
                    bio: profileData.bio || '',
                    followers: profileData.followers || Math.floor(Math.random() * 5000),
                    following: profileData.following || Math.floor(Math.random() * 500),
