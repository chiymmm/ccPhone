        ...
        if(this.currentBookId) {
            ...
            book.entries.forEach(e => {
                ...
                div.innerHTML = `
                    ...
                    <div class="wb-entry-preview">${e.content}</div>
                `;
                div.onclick = () => this.openEntryEditor(e);
            ...
            // Render Books List
            document.getElementById('wbTitle').innerText = '世界书';
            document.getElementById('wbBackBtn').style.display = 'none'; // Should be handled by app container back logic if needed, but here we use it for navigation inside app
    ...
    importBook(file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                // Support TavernAI format (entries array) or simple format
                let entries = [];
                let name = file.name.replace('.json', '');

                if(Array.isArray(json)) {
                    // TavernAI export usually is a dictionary or array of entries
                    // Assuming simple array of {key: [], content: ""} for now or Tavern format
                    // Tavern format: { entries: { "uid": { key: [], content: "" } } }
                    // Or Character card embedded lorebook
                } 
                
                // Try to parse standard TavernAI Lorebook
                if(json.entries) {
