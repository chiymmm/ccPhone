...
class InstagramApp {
    ...
    initUI() {
        ...
        if(!document.getElementById('igChatWindow')) {
            ...
            chatWin.innerHTML = `
                ...
                <div class="chat-input-area">
                    <button class="chat-tool-btn" id="igGenChatBtn"><i class="fas fa-magic"></i></button>
        ...
        if(!document.getElementById('btnGenIgFeed')) {
            const btn = document.createElement('div');
            btn.id = 'btnGenIgFeed';
            btn.style.cssText = 'padding:10px; text-align:center; color:#0095f6; font-weight:bold; cursor:pointer; border-bottom:1px solid #dbdbdb;';
            btn.innerHTML = '<i class="fas fa-magic"></i> 生成推荐流 (AI)';
            btn.onclick = () => this.generateFeed();
            const homeTab = document.getElementById('ig-home');
            homeTab.insertBefore(btn, homeTab.firstChild);
        }
    ...
    async generateFeed() {
        ...
        const btn = document.getElementById('btnGenIgFeed');
        ...
        const prompt = `生成 3 条 Instagram 风格的帖子。
        要求：
    ...
    async generateActivity() {
        ...
        try {
            ...
            this.store.update(d => {
                d.posts.push({
                    id: window.Utils.generateId('ig'),
                    userId: 'me', // Character is "me" in this context
                    username: char.name, // Or use profile username if set
                    avatar: char.avatar,
                    image: imgId,
    ...
    renderDMs() {
        ...
        data.dms.forEach(dm => {
            const div = document.createElement('div');
            div.className = 't-dm-item'; // Reuse twitter style
