...
class API {
    ...
    static async callAI(messages, config = null) {
        ...
        // Handle Vision API (Image Content)
        // If content is array, it's likely already formatted for Vision, or we need to ensure it is.
        // We don't need to do much if the caller constructs it correctly.
        // But we should ensure the model supports vision if images are present.
        // For now, we assume the user selects a capable model (like gpt-4o) in settings.
        ...
        try {
            const res = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    messages: msgs,
                    temperature: 0.7,
                    max_tokens: 4096 // Increase limit for longer responses
                })
            });
            ...
            // alert(`AI 生成失败: ${e.message}`); // Optional: suppress alert to avoid spamming
            throw e;
        }
    }
