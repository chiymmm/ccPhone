...
// ========== X默认头像生成 ==========
// 在 window.Utils 对象中添加或者直接全局添加

window.Utils = window.Utils || {};

// X风格默认头像（灰色蛋头）
window.Utils.generateXDefaultAvatar = function() {
    const canvas = document.createElement('canvas');
    canvas.width = 200;
    canvas.height = 200;
    const ctx = canvas.getContext('2d');
    ...
    ctx.beginPath();
    ctx.ellipse(100, 85, 45, 50, 0, 0, Math.PI * 2);
    ctx.fill();

    // 身体
    ctx.beginPath();
    ctx.ellipse(100, 180, 60, 45, 0, Math.PI, 0);
    ctx.fill();
...
// 判断头像来源并获取正确头像
window.Utils.getCharacterAvatar = async function(character, source) {
    // source: 'qq' | 'x' | 'custom'

    if(character.customAvatar) {
        // 用户自定义头像优先
        let avatar = character.customAvatar;
    ...
    if(source === 'qq') {
        ...
        const friend = (qqData.friends || []).find(f =>
            f.id === character.qqId ||
    ...
    if(source === 'x' || !source) {
        // X原生角色使用X默认头像
        return window.Utils.generateXDefaultAvatar();
    }

    // 兜底：生成彩色默认头像
    return window.Utils.generateDefaultAvatar(character.name || 'User');
};
